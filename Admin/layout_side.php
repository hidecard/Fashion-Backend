
<div id="layoutSidenav_nav">
    <nav class="sb-sidenav accordion text-black card" id="sidenavAccordion">
        <div class="sb-sidenav-menu">
            <div class="nav">
                <div class="sb-sidenav-menu-heading">Core</div>
                <a class="nav-link text-black" href="index.php">
                    <div class="sb-nav-link-icon"><i class="fa-solid fa-house"></i></div>
                    Dashboard
                </a>
                <div class="sb-sidenav-menu-heading">Components</div>
                <a class="nav-link text-black" href="user_list.php">
                    <div class="sb-nav-link-icon"><i class="fa-solid fa-users"></i></div>
                    User Lists
                </a>
                <a class="nav-link text-black" href="category_list.php">
                    <div class="sb-nav-link-icon"><i class="fa-solid fa-list"></i></div>
                    Category Lists
                </a>
                <a class="nav-link text-black" href="pro_list.php">
                    <div class="sb-nav-link-icon"><i class="fa-brands fa-product-hunt"></i></div>
                    Product Lists
                </a>
                <a class="nav-link text-black" href="order_list.php">
                    <div class="sb-nav-link-icon"><i class="fa-solid fa-cart-shopping"></i></div>
                    Order Lists
                </a>
                <a class="nav-link text-black" href="message.php">
                    <div class="sb-nav-link-icon"><i class="fa-regular fa-message"></i></div>
                    View Messages
                </a>
                <a class="nav-link text-black" href="balance.php">
                    <div class="sb-nav-link-icon"><i class="fa-solid fa-money-bills"></i></div>
                    Balance
                </a>
            </div>
        </div>
    </nav>
</div>